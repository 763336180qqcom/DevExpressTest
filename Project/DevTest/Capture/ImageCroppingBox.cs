    ...
    public class ImageCroppingBox : Control
    {
        ...
        public bool IsLockSelected {
            ...
            set {
                ...
                if (value) this.Cursor = Cursors.Default;
            }
        }
        ...
        public Rectangle PreViewRectangle {
            ...
            set {
                ...
                if (this._PreViewRectangle == value) return;
                this._PreViewRectangle = value;
                this.Invalidate();
            }
        }
        ...
        protected override void OnMouseDown(MouseEventArgs e) {
            ...
            if (this._IsSelected) {
                this._IsSelected = false; //默认表示 要更改选取设置 清楚IsSelected属性
                if (m_rectDots[0].Contains(e.Location)) {               //左上点
                    m_ptStart.X = this._SelectedRectangle.Right - 1;
                    m_ptStart.Y = this._SelectedRectangle.Bottom - 1;
                } else if (m_rectDots[1].Contains(e.Location)) {        //上
                    m_ptStart.Y = this._SelectedRectangle.Bottom - 1;
                    m_bChangeWidth = false;
                } else if (m_rectDots[2].Contains(e.Location)) {        //右上
                    m_ptStart.X = this._SelectedRectangle.X;
                    m_ptStart.Y = this._SelectedRectangle.Bottom - 1;
                } else if (m_rectDots[3].Contains(e.Location)) {        //左边
                    m_ptStart.X = this._SelectedRectangle.Right - 1;
                    m_bChangeHeight = false;
                } else if (m_rectDots[4].Contains(e.Location)) {        //右边
                    m_ptStart.X = this._SelectedRectangle.X;
                    m_bChangeHeight = false;
                } else if (m_rectDots[5].Contains(e.Location)) {        //左下
                    m_ptStart.X = this._SelectedRectangle.Right - 1;
                    m_ptStart.Y = this._SelectedRectangle.Y;
                } else if (m_rectDots[6].Contains(e.Location)) {        //下
                    m_ptStart.Y = this._SelectedRectangle.Y;
                    m_bChangeWidth = false;
                } else if (m_rectDots[7].Contains(e.Location)) {        //右下
                    m_ptStart = this._SelectedRectangle.Location;
                } else if (this._SelectedRectangle.Contains(e.Location)) {
                    m_bMoving = true;
                    m_bChangeWidth = false;
                    m_bChangeHeight = false;
                } else { this._IsSelected = true; }   //若以上条件不成立 表示不需要更改设置 
            }//上面至于为什么要减去1 可以去掉试一下效果
            base.OnMouseDown(e);
        }
        ...
        protected override void OnMouseMove(MouseEventArgs e) {
            ...
            if (this._IsSelected) {//如果已经绘制 移动过程中判断是否需要设置鼠标样式
                ...
                if (m_bChangeWidth) {//是否允许选区宽度改变 如重置大小时候 拉动上边和下边中点时候
                    ...
                    this._SelectedRectangle.Width = Math.Abs(e.Location.X - m_ptStart.X) + 1;
                }
                if (m_bChangeHeight) {
                    this._SelectedRectangle.Y = e.Location.Y > m_ptStart.Y ? m_ptStart.Y : e.Location.Y;
                    this._SelectedRectangle.Height = Math.Abs(e.Location.Y - m_ptStart.Y) + 1;
                }
                if (m_bMoving) {//如果是移动选区 判断选区移动范围
                ...
                this.Invalidate();
            } else if (this._IsDrawMagnifier && !this._IsSelected) {
        ...
        protected override void OnPaint(PaintEventArgs e) {
            ...
            if (this._Image != null) {
                ...
                if (!this._SelectedRectangle.IsEmpty)
                    ...
                    this.PreViewRect(g);
                if (this._IsDrawMagnifier && m_bMouseHover && !_IsSelected && !m_bMoving)
        ...
        protected virtual void PreViewRect(Graphics g) {
            g.DrawImage(this._Image, this._PreViewRectangle, this._PreViewRectangle, GraphicsUnit.Pixel);
            using (Pen pen = new Pen(Color.Cyan, 3)) {
                g.DrawRectangle(pen, this._PreViewRectangle.X, this._PreViewRectangle.Y, this._PreViewRectangle.Width - 1, this._PreViewRectangle.Height - 1);
            }
        }
